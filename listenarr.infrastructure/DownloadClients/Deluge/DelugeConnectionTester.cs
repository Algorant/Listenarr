...
{
    internal sealed class DelugeConnectionTester(DelugeRpcClient rpcClient, ILogger logger)
    {
        public async Task<(bool Success, string Message)> TestConnectionAsync(DownloadClientConfiguration client, CancellationToken ct = default)
        {
            try
            {
                ...
                return connected.ValueKind == System.Text.Json.JsonValueKind.True
                    ? (true, "Deluge: connected to Web UI and daemon")
                    : (false, "Deluge: unexpected JSON-RPC response");
            }
